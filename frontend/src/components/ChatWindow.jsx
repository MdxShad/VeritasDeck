...
const ChatWindow = () => {
  ...
  useEffect(() => {
    const savedConversations = localStorage.getItem('veritasdeck_conversations')
  ...
  useEffect(() => {
    if (conversations.length > 0) {
      ...
      localStorage.setItem('veritasdeck_conversations', JSON.stringify(cleanConversations))
    }
  }, [conversations])
  ...
  const handleDeleteConversation = useCallback((convId) => {
    setConversations(prev => {
      ...
      if (filtered.length === 0) {
        localStorage.removeItem('veritasdeck_conversations')
      }
      return filtered
    })
