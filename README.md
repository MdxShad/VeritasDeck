...
<img src="assets/veritasdeck_logo.svg" alt="VeritasDeck Logo" width="200"/><br>

# VeritasDeck: From Paper to Presentation in One Click
...
python -m veritasdeck --input paper.pdf --output slides --style doraemon --length medium --fast --parallel 2
```
...
- **[2025.12.08]** VeritasDeck is now open source!
...
- [🏗️ VeritasDeck Framework](#%EF%B8%8F-veritasdeck-framework)
...
conda create -n veritasdeck python=3.12 -y
conda activate veritasdeck
...
> [!NOTE]
> Create a `.env` file in `veritasdeck/` directory with your API keys. Refer to `veritasdeck/.env.example` for the required variables.

### 2. Command Line Usage

```bash
# Basic usage - generate slides from a paper
python -m veritasdeck --input paper.pdf --output slides --length medium

# Generate poster with custom style
python -m veritasdeck --input paper.pdf --output poster --style "minimalist with blue theme" --density medium

# Fast mode
python -m veritasdeck --input paper.pdf --output slides --fast

# Enable parallel generation (2 workers by default)
python -m veritasdeck --input paper.pdf --output slides --parallel 2

# List all processed outputs
python -m veritasdeck --list
```
...
VeritasDeck intelligently saves your progress at every key stage, allowing you to:
...
## 🏗️ VeritasDeck Framework

VeritasDeck transforms documents through a 4-stage pipeline designed for **reliability** and **efficiency**:
...
> [!TIP]
> VeritasDeck uses `gemini-3-pro-image-preview` (Nano Banana Pro Preview) for image generation. Key findings:
> 
...
| Module | Description |
|--------|-------------|
| `veritasdeck/core/` | Pipeline orchestration, 4-stage execution |
| `veritasdeck/raganything/` | Document parsing & RAG indexing |
| `veritasdeck/summary/` | Content extraction: figures, tables, paper structure |
| `veritasdeck/generator/` | Content planning & image generation |
...
**🌟Found VeritasDeck helpful? Star us on GitHub!**
